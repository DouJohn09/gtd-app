...
- [ ] **CRITICAL — SQL injection in `ProjectModel.update`** (`server/src/db/models.js:458-471`). Builds `SET ${field}` from raw `Object.keys(req.body)` (via `routes/projects.js:48`); unlike the other update paths it has no whitelist. Any authenticated user can craft a body key to exfiltrate the whole DB (emails, Google refresh tokens, Paddle IDs) or plant rows in another user's account (`user_id` injection). **Fix:** whitelist `['name','description','status','outcome','execution_mode']`.
- [ ] **CRITICAL — Cross-user FK binding** (`models.js:125-157,160,672-683`; `routes/projects.js:87`). `project_id`/`list_id` accepted with no ownership check; SERIAL ids are enumerable → leaks other users' project names via the `LEFT JOIN projects` (no `p.user_id` filter), and attacker list-items get cascade-deleted with the victim's list. **Fix:** verify FK ownership on create/update.
- [ ] **P0 — Silent login failure** (`client/src/pages/Login.jsx:56`; `AuthContext.jsx:32`). `login()` throws on non-OK response, nothing catches it. Non-allowlisted beta users complete the Google popup → nothing happens, no message. First screen of the beta. **Fix:** catch + show error state ("this app is in private beta").
- [ ] **P0 — 401 redirect goes to wrong path in prod** (`client/src/lib/api.js:26`). Redirects to `/login`, but prod serves the app at `base: '/app/'`. Expired token ejects the user to the landing domain. **Fix:** `/app/login` (use `import.meta.env.BASE_URL`).
- [ ] **P0 — Capacity-gate 402s handled nowhere** (`Projects.jsx:89-177` naked awaits; `Habits.jsx:200`; NewListModal; `api.js:41-46` sets `err.code` but no caller reads it except `BillingSection`). Gates are LIVE in prod → Free user at limit clicks Create → unhandled rejection, dead button. Revenue loss at the exact upgrade moment. **Fix:** catch `err.code==='limit_reached'` → upgrade modal.
- [ ] **HIGH — Legal placeholders live in prod.** `landing-page/privacy.html` §1 and `terms.html` §1 contain `[registered address — fill in]`. Terms §14 links the **defunct** EU ODR platform (discontinued Jul 2025) → replace with ČOI (adr.coi.cz). Sub-processor table omits **Cloudflare**. Confirm `support@cleartable.app` actually receives mail. Blocks Paddle approval + GDPR.
- [ ] **HIGH — False competitive claim** on landing: "only cleartable + omnifocus have this" (guided weekly review). FacileThings/Nirvana also have it; r/gtd knows. Soften to "almost no task app has this." Same pass: FAQ says PWA "shipping before public launch" (shipped 2026-05-31 — say "installable today"); schema `operatingSystem: "Web, iOS, Android"` → "Web (PWA)"; "AI runs your weekly review" → "prepares your review."
...
### Compliance & security
- [ ] **HIGH — No account deletion (GDPR Art. 17).** No route exists; and child tables reference `users(id)` with **no `ON DELETE` action** (`migrations/1779667200000_initial-schema.js:39,70,87,102,114`), so even manual `DELETE FROM users` fails on FK violations. EU controller selling to EU users. **Fix:** delete-account path that cascades data + cancels Paddle sub + revokes Google token.
...
- [ ] **HIGH — No rate limiting on AI endpoints + metering OFF + `aiLimit` fails open** (`middleware/aiLimit.js:25-28`; `services/aiUsage.js:9-12`). Only `/auth/google` is limited. Scripted free account loops `smart-capture` → Groq free tier saturates → every call fails over to **paid OpenAI** → bill scales with attacker. `find-duplicates` is the ~$0.10-0.30/call variant. **Fix:** turn on `AI_DAILY_LIMIT_FREE`, express-rate-limit `/api/ai/*` + `/api/custom-lists/extract-url`, fail closed for cost endpoints.
...
- [ ] **HIGH — UTC-anchored "today"** breaks habits, streaks, daily focus, deferred tasks, recurrence for any non-UTC user (incl. the founder in Prague). `habits.js:17,66,306`; `models.js:263-267` + all `CURRENT_DATE`. Client already sends `X-Client-Timezone` (only used for GCal). **Fix:** compute "today" in the user's tz.
