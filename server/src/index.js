...
// TEMPORARY: one-time data recovery endpoint to extract the legacy
// /data/gtd.db (sql.js era) so we can migrate it to Postgres. Protected by
// the RECOVERY_TOKEN env var. REMOVE this route + the env var as soon as
...
app.get('/__recovery/gtd-db', (req, res) => {
  ...
  if (!existsSync(path)) {
    return res.status(404).send('Legacy gtd.db not present');
  }
